...
thermal_morph <- read.csv(file = "Summary_sheets/Aus_Thermal_beh_traits_by_Spp.csv") %>% 
  filter(Genus_species== "Ctenophorus_isolepis")

shape <- 3 # lizard
Ww_g <- thermal_morph$mass_g # in between W_p and W_i
CT_min <- thermal_morph$CT_min # MK data critical thermal minimum (deg C)
