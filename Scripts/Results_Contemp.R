...
Ta_Tb_contemp_africa <- ggplot(contemp_ta_tb_africa, aes(x = Combined, y = Slope_10_Year, 
  ...
  geom_point(position = position_dodge(width = .3), size = 5, alpha = 0.5, stroke = 0.9) +
...
overall_Mrate_change_value <-  MRate_contemp_plot_dat%>% 
  group_by(Region) %>% 
  summarise(Mrate_mean_decade = mean(Slope_10_Year))
...
contemp_Foraging_winter_plot_2 <- ggplot(contemp_Foraging_winter_plot_dat, 
  ...
  guides(fill = FALSE, color = guide_legend(order = 1), shape = guide_legend(order = 2)) +
  theme_bw() +
  scale_y_continuous(limits = c(-1.99, -.73), breaks = seq(-1.8, -.8, by = .4))  +
  labs(y = NULL, 
       color = "Species", 
       x = NULL)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
ggsave("../../Pianka_figs/Figure_3.pdf", Figure_3, device = "pdf", width = 26, height =14)
