...
thermal_morph <- read.csv(file = "Summary_sheets/KL_Thermal_beh_traits_by_Spp.csv") %>% 
  filter(Genus_species== "Pedioplanis_lineoocellata")
...
T_B_min <- thermal_morph$T_B_min # minimum basking body temperature (deg C) observed in field - ******* Tb_min value is smaller than RB CHECK
...
CT_max <-  thermal_morph$CT_max # critical thermal maximum (deg C) 
